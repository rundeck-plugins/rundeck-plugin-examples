...
Each example is a real, buildable Gradle or Maven project, and each is scanned by Snyk
through the org-wide security workflow. Because these examples are meant to be copied as
starting points, they aim to model current, patched dependency versions: reported
vulnerabilities are **fixed** (by upgrading or overriding the affected dependency)
rather than suppressed. See [SECURITY.md](SECURITY.md) for how findings are handled.

⚠️ **Important:** Example plugins are for educational purposes. Always perform a security
review and testing before deploying any plugin to a production Rundeck environment.
